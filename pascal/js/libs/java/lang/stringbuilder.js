export class StringBuilder {
  ...
  charAt(index) {
    ...
    if (i < 0 || i >= this._s.length) throw new RangeError("StringBuilder.charAt: index out of range");
    return this._s.charAt(i);
  }

  setLength(newLength) {
    const n = Number(newLength) | 0;
    if (n < 0) throw new RangeError("StringBuilder.setLength: newLength must be >= 0");
  ...
  insert(index, str) {
    const i = Number(index) | 0;
    if (i < 0 || i > this._s.length) throw new RangeError("StringBuilder.insert: index out of range");
    const t = String(str);
  ...
  delete(start, end) {
    ...
    if (s < 0 || e < s || s > this._s.length) throw new RangeError("StringBuilder.delete: invalid range");
